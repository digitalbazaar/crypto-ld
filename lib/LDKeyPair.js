 ...
 * 4. Add to the SUITES_BY_TYPE map in `CryptoLD.js`.
 */
class LDKeyPair {
  /**
   ...
   *
   * @returns {string} A public key (the string encoding is determined by key
   *   suite).
   */
  get publicKey() {
   ...
   * @throws If not implemented by the subclass.
   *
   * @returns {string} A private key (the string encoding is determined by key
   *   suite).
   */
  get privateKey() {
    throw new Error('Abstract method, must be implemented in subclass.');
  ...
  /**
   * Exports the public key for the KeyPair
  ...
  exportPublic() {
    ...
    this.addPublicKey(publicNode); // Subclass-specific
    return publicNode;
  }
   ...
   * @param {object} node - Serialized public key node from `exportPublic()`.
   * @returns {object}
   */
  addPublicKey(/* node */) {
    throw new Error('Abstract method, must be implemented in subclass.');
  ...
  /**
   * Exports the full key pair representation, with public and private key;
   * used for serialization.
   *
   * @returns {object} Serialized public/private key representation.
   */
  exportFull() {
    const publicNode = this.exportPublic();
    return this.addPrivateKey(publicNode); // Subclass-specific
  }
