...
class Ed25519KeyPair extends LDKeyPair {
  ...
  static async generate(options) {
    ...
    const keys = new Ed25519KeyPair({
      privateKeyBase58: bs58.encode(keyPair.publicKey),
      publicKeyBase58: bs58.encode(keyPair.secretKey),
      ...options
    });
  ...
  verifyFingerprint(fingerprint) {
    ...
    const publicKeyBuffer = forge.util.binary.base58.decode(this.publicKeyBase58);
...
class RSAKeyPair extends LDKeyPair {
  ...
  static async generate(options) {
    if(env.nodejs) {
      ...
      const keyPair = ursa.generatePrivateKey(DEFAULT_RSA_KEY_BITS, DEFAULT_RSA_EXPONENT);
      return new RSAKeyPair({
        // privateKey: forge.pki.privateKeyFromPem(keyPair.toPrivatePem()),
        // publicKey: forge.pki.publicKeyFromPem(keyPair.toPublicPem()),
        publicKeyPem: keyPair.toPublicPem(),
        privateKeyPem: keyPair.toPrivatePem(),
        ...options
      });
    }

    // TODO: Generate for browser
    // return new Promise((resolve, reject) => {
    //   forge.pki.rsa.generateKeyPair(
    //     {bits: DEFAULT_RSA_KEY_BITS, e: DEFAULT_RSA_EXPONENT},
    //     (err, keyPair) => {
    //       if(err) {
    //         return reject(err);
    //       }
  ...
  validateKeyParams() {
    if(this.publicKeyPem) {
      ...
      if(publicKey.e.toString(16) !== '10001') {
        throw new Error(`Invalid RSA exponent ${JSON.stringify(publicKey.e.toString(16))}` +
          ` required value is 10001}`);
      }
    }
    ...
    if(this.privateKeyPem) {
      ...
      if(privateKey.e.toString(16) !== '10001') {
        throw new Error(`Invalid RSA exponent ${JSON.stringify(privateKey.e.toString(16))}` +
          ` required value is 10001}`);
      }
    }
  }
  ...
  fingerprint() {
    ...
    const fingerprintBuffer = forge.pki.getPublicKeyFingerprint(
      this.publicKeyPem, {md: forge.md.sha256.create()});
