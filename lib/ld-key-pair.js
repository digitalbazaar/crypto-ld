...
class Ed25519KeyPair extends LDKeyPair {
  ...
  async encrypt(privateKey, password) {
    ...
    const jweHeader = {
      ...
      jwk: {
        kty: 'PBKDF2',
        s: forge.util.encodeBase64Url(salt, {forge}),
        c: iterations
      }
    };
    ...
    const jwe = {
      ...
      iv: forge.util.encodeBase64Url(iv, {forge}),
      ciphertext: forge.util.encodeBase64Url(encrypted, {forge}),
      tag: forge.util.encodeBase64Url(tag, {forge})
    };
  ...
  async decrypt(jwe, password) {
    ...
    salt = forge.util.decodeBase64Url(salt, {forge});
    const key = await LDKeyPair.pbkdf2(password, salt, iterations, keySize);

    const iv = forge.util.decodeBase64Url(jwe.iv, {forge});
    const tag = forge.util.decodeBase64Url(jwe.tag, {forge});
    const decipher = forge.cipher.createDecipher('AES-GCM', key);
    decipher.start({iv, tag});
    decipher.update(forge.util.decodeBase64Url(jwe.ciphertext, {forge}));
