...
class CryptoLD {
   ...
   *
   * @param {string} keyId - Key ID or URI.
   * @param {Function} documentLoader - JSON-LD Document Loader.
   * @param {boolean} [checkContext=true] - Whether to check that the fetched
   *   key document contains the context required by the key's crypto suite.
   * @param {boolean} [checkRevoked=true] - Whether to check the key object for
   *   the presence of the `revoked` timestamp.
   *
  ...
  async fromKeyId({
    ...
    if(!keyId) {
      throw new TypeError('The "keyId" parameter is required.');
    }
    if(!documentLoader) {
    ...
    if(checkRevoked && keyDocument.revoked) {
      ...
      throw new Error(`Key has been marked as revoked: "${keyDocument.revoked}".`);
    }
